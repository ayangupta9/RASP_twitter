...
import type { FeedbackValue, Platform } from "@/src/lib/types"
...
export function PopupContent() {
  ...
  useEffect(() => {
    ...
    const setupConnection = () => {
      ...
      chrome.runtime.sendMessage({ type: "GET_LAST_IMAGE_AND_PREDICTION" }, (response: any) => {
        if (!mounted) return
    ...
    <div className="w-[400px] max-h-[600px] bg-background p-4">
      <ScrollArea className="h-full">
        <div>
          ...
          <PredictionResult prediction={prediction} loading={loading} progress={progress} />

          {showConfirmationDialog && <ConfirmationDialog handleConfirm={handleConfirm} handleReject={handleReject} />}

          {!loading && prediction && !showConfirmationDialog && (
            <>
              {!feedbackComplete && (
                <FeedbackForm
                  image={image}
